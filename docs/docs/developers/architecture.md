...
- **`core/`** — the brains:
    ...
    - **`core/client.go` — `IrcClient` + `Handlers`: the single high-level API
      every interface uses.** See [Adding an Interface](./adding-an-interface.md).
...
!!! tip "Golden rule"
    Never reimplement IRC, DCC, or search-result parsing inside an interface.
    Wire up `core.IrcClient` and supply `Handlers`. See
    [Adding an Interface](./adding-an-interface.md).
...
## Connection models: current and future

Historically each user gets their own IRC connection so responses can be routed
back to the right websocket:

```mermaid
graph LR
  A[User] -->|Websocket| B{OpenBooks};
  X[User] -->|Websocket| B;
  B -->|IRC conn 1| C[IRC Highway];
  B -->|IRC conn 2| C[IRC Highway];
```

The long-term goal is a **single** IRC connection regardless of client count,
routing each IRC response to the correct client. The hard part is correlating
responses, since IRC messages carry no request id.

```mermaid
graph LR
  A[User] -->|Websocket| B{OpenBooks};
  X[User] -->|Websocket| B;
  B -->|single IRC conn| C[IRC Highway];
```

The Discord bot's `Broker` is the first realization of this shared-connection
model: it serializes requests over one connection and correlates each reply to
the waiting caller. See
[Adding an Interface](./adding-an-interface.md#one-connection-vs-many).
